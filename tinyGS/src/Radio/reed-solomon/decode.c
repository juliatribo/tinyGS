...
static bool reed_solomon_find_syndromes(field_t field, polynomial_t_rs msgpoly, field_logarithm_t **generator_root_exp,
...
// Chien search
bool reed_solomon_factorize_error_locator(field_t field, unsigned int num_skip, polynomial_t_rs locator_log, field_element_t *roots,
...
void reed_solomon_find_error_evaluator(field_t field, polynomial_t_rs locator, polynomial_t_rs syndromes,
                                       polynomial_t_rs error_evaluator) {
...
static polynomial_t_rs reed_solomon_find_error_locator_from_roots(field_t field, unsigned int num_errors,
                                                               field_element_t *error_roots,
                                                               polynomial_t_rs error_locator,
                                                               polynomial_t_rs *scratch) {
...
// erasure method
static void reed_solomon_find_modified_syndromes(correct_reed_solomon *rs, field_element_t *syndromes, polynomial_t_rs error_locator, field_element_t *modified_syndromes) {
    polynomial_t_rs syndrome_poly;
    syndrome_poly.order = rs->min_distance - 1;
    syndrome_poly.coeff = syndromes;

    polynomial_t_rs modified_syndrome_poly;
...
ssize_t correct_reed_solomon_decode_with_erasures(correct_reed_solomon *rs, const uint8_t *encoded,
    ...
    */
    ...
    polynomial_t_rs temp_poly = polynomial_create(rs->error_locator.order + erasure_length);
    polynomial_mul(rs->field, rs->erasure_locator, rs->error_locator, temp_poly);
    polynomial_t_rs placeholder_poly = rs->error_locator;
